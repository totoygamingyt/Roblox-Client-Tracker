...
void main()
{
    ...
    vec3 f19 = (((VARYING6.xyz * f16) + min((f12.xyz * (f12.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f13.x)), vec3(CB0[21].w))) * mix(f7.xyz, textureLod(PrefilteredEnvTexture, vec4(f17, f18).xyz, f18).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f17.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(VARYING7.w))) + (CB0[15].xyz * ((VARYING6.w * f16) * 0.100000001490116119384765625));
    vec4 f20 = vec4(0.0);
    f20.x = f19.x;
    vec4 f21 = f20;
    f21.y = f19.y;
