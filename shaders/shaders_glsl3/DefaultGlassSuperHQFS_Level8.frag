...
void main()
{
    ...
    float f34 = f32.x;
    float f35 = f32.y;
    vec3 f36 = f25 - CB0[46].xyz;
    vec3 f37 = f25 - CB0[47].xyz;
    vec3 f38 = f25 - CB0[48].xyz;
    vec4 f39 = vec4(f25, 1.0) * mat4(CB8[((dot(f36, f36) < CB0[46].w) ? 0 : ((dot(f37, f37) < CB0[47].w) ? 1 : ((dot(f38, f38) < CB0[48].w) ? 2 : 3))) * 4 + 0], CB8[((dot(f36, f36) < CB0[46].w) ? 0 : ((dot(f37, f37) < CB0[47].w) ? 1 : ((dot(f38, f38) < CB0[48].w) ? 2 : 3))) * 4 + 1], CB8[((dot(f36, f36) < CB0[46].w) ? 0 : ((dot(f37, f37) < CB0[47].w) ? 1 : ((dot(f38, f38) < CB0[48].w) ? 2 : 3))) * 4 + 2], CB8[((dot(f36, f36) < CB0[46].w) ? 0 : ((dot(f37, f37) < CB0[47].w) ? 1 : ((dot(f38, f38) < CB0[48].w) ? 2 : 3))) * 4 + 3]);
    ...
    float f44 = exp(CB0[51].z * f43);
    float f45 = -exp((-CB0[51].w) * f43);
    vec2 f46 = (f42 * CB0[52].y) * vec2(f44, f45);
    vec2 f47 = f46 * f46;
    float f48 = f40.x;
    ...
    float f58 = CB0[31].w * f0;
    float f59 = max(9.9999997473787516355514526367188e-05, dot(f22, f56));
    vec3 f60 = reflect(-f56, f22);
    float f61 = f57 * 5.0;
    vec3 f62 = vec4(f60, f61).xyz;
    vec3 f63 = textureLod(PrefilteredEnvTexture, f62, f61).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f60.y * 1.58823525905609130859375, 0.0, 1.0)));
    ...
    {
        f65 = mix(f64, textureLod(PrefilteredEnvBlendTargetTexture, f62, f61).xyz, vec3(CB0[32].w));
    }
    vec4 f66 = texture(PrecomputedBRDFTexture, vec2(f57, f59));
    float f67 = CB0[14].w * CB0[14].w;
    vec3 f68 = normalize(f56 - CB0[16].xyz);
    float f69 = clamp((f24 * CB0[14].w) * (((f24 * CB0[52].x) > 0.0) ? mix(min((f44 <= f48) ? 1.0 : clamp(((f49 / (f49 + (f50 * f50))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f45 <= f51) ? 1.0 : clamp(((f52 / (f52 + (f53 * f53))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f35, clamp((length(f25 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f35), 0.0, 1.0);
    float f70 = f57 * f57;
    ...
    vec4 f98 = vec4(CB0[11].xyz - VARYING4.xyz, 1.0) * f97;
    vec4 f99 = vec4(CB0[11].xyz - ((VARYING4.xyz * (1.0 + ((3.0 * f96) / max(dot(VARYING4.xyz, f22), 0.00999999977648258209228515625)))) + (f22 * (3.0 * (1.0 - f96)))), 1.0) * f97;
    float f100 = f98.w;
    ...
    vec2 f112 = normalize(f111) * CB0[28].x;
    ...
    vec4 f124 = mix(mix(f123, vec4(mix(((((((vec3(1.0) - (f76 * f58)) * CB0[15].xyz) * f69) + (f33 * f17)) + (((vec3(1.0) - (f83 * f58)) * (((((((CB0[40].xyz * f88) + (CB0[42].xyz * f89)) + (CB0[44].xyz * f90)) + (CB0[41].xyz * f91)) + (CB0[43].xyz * f92)) + (CB0[45].xyz * f93)) + (((((((CB0[34].xyz * f88) + (CB0[36].xyz * f89)) + (CB0[38].xyz * f90)) + (CB0[35].xyz * f91)) + (CB0[37].xyz * f92)) + (CB0[39].xyz * f93)) * f34))) * f17)) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f34))) * (f54 * f17), f80, vec3(VARYING7.w)), 1.0), vec4(f17)), vec4(f80, 1.0), vec4(((f94 * f94) * 0.800000011920928955078125) * f18)) + vec4((((f76 * (((f77 + (f77 * f77)) / (((f78 * f78) * ((f72 * 3.0) + 0.5)) * ((f71 * 0.75) + 0.25))) * f69)) * CB0[15].xyz) * f67) + (((mix(f65, f63, f79) * f83) * f58) * (f67 * f17)), 0.0);
    float f125 = clamp(exp2((CB0[18].z * f55) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f126 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[18].y, f125) * 5.0).xyz;
    ...
    vec4 f132 = f131;
    f132.w = f125 * f124.w;
    vec3 f133 = sqrt(clamp(f132.xyz * CB0[20].y, vec3(0.0), vec3(1.0)));
    vec4 f134 = f132;
    f134.x = f133.x;
    vec4 f135 = f134;
    f135.y = f133.y;
