...
void main()
{
    ...
    vec3 f43 = VARYING5.xyz - (CB0[16].xyz * 0.001000000047497451305389404296875);
    float f44 = clamp(dot(step(CB0[24].xyz, abs(VARYING4 - CB0[23].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f58 = CB0[31].w * f34;
    vec3 f59 = reflect(-f54, f42);
    vec3 f60 = -CB0[16].xyz;
    float f61 = (dot(f42, f60) * CB0[14].w) * ((1.0 - ((step(f52.x, f53) * clamp(CB0[29].z + (CB0[29].w * abs(f53 - 0.5)), 0.0, 1.0)) * f52.y)) * f50.y);
    vec3 f62 = normalize(f54 - CB0[16].xyz);
    ...
    float f92 = f86.y;
    float f93 = f86.z;
    vec3 f94 = (((((((((f75 - (f70 * f74)) * CB0[15].xyz) * f63) + (CB0[17].xyz * (f73 * clamp(-f61, 0.0, 1.0)))) + ((f49.xyz * (f49.w * 120.0)) * 1.0)) + ((f75 - (f83 * f74)) * (((((((CB0[40].xyz * f88) + (CB0[42].xyz * f89)) + (CB0[44].xyz * f90)) + (CB0[41].xyz * f91)) + (CB0[43].xyz * f92)) + (CB0[45].xyz * f93)) + (((((((CB0[34].xyz * f88) + (CB0[36].xyz * f89)) + (CB0[38].xyz * f90)) + (CB0[35].xyz * f91)) + (CB0[37].xyz * f92)) + (CB0[39].xyz * f93)) * f51)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f51))) + vec3((f28.z * 2.0) * f34)) * f33) + ((((((f70 * (((f71 + (f71 * f71)) / (((f72 * f72) * ((f66 * 3.0) + 0.5)) * ((f65 * 0.75) + 0.25))) * f63)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f34) * VARYING0.w) + ((mix(f79, textureLod(PrefilteredEnvTexture, f77, f76).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f59.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f51)) * f83) * f58));
    vec4 f95 = vec4(0.0);
    f95.x = f94.x;
    vec4 f96 = f95;
    f96.y = f94.y;
