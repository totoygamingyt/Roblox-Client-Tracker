...
void main()
{
    ...
    vec3 f25 = VARYING5.xyz - (CB0[16].xyz * 0.001000000047497451305389404296875);
    float f26 = clamp(dot(step(CB0[24].xyz, abs(VARYING4 - CB0[23].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f55 = CB0[31].w * f21;
    vec3 f56 = reflect(-f53, f22);
    vec3 f57 = normalize(f53 - CB0[16].xyz);
    float f58 = clamp((f24 * CB0[14].w) * (((f24 * CB0[52].x) > 0.0) ? mix(min((f43 <= f47) ? 1.0 : clamp(((f48 / (f48 + (f49 * f49))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f44 <= f50) ? 1.0 : clamp(((f51 / (f51 + (f52 * f52))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f34, clamp((length(f25 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f34), 0.0, 1.0);
    float f59 = f54 * f54;
    ...
    float f84 = f78.y;
    float f85 = f78.z;
    vec3 f86 = ((((((((vec3(1.0) - (f65 * f55)) * CB0[15].xyz) * f58) + ((f31.xyz * (f31.w * 120.0)) * 1.0)) + ((vec3(1.0) - (f75 * f55)) * (((((((CB0[40].xyz * f80) + (CB0[42].xyz * f81)) + (CB0[44].xyz * f82)) + (CB0[41].xyz * f83)) + (CB0[43].xyz * f84)) + (CB0[45].xyz * f85)) + (((((((CB0[34].xyz * f80) + (CB0[36].xyz * f81)) + (CB0[38].xyz * f82)) + (CB0[35].xyz * f83)) + (CB0[37].xyz * f84)) + (CB0[39].xyz * f85)) * f33)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f33))) + vec3((f5.z * 2.0) * f21)) * (f19.xyz * f19.xyz)) + ((((((f65 * (((f66 + (f66 * f66)) / (((f67 * f67) * ((f61 * 3.0) + 0.5)) * ((f60 * 0.75) + 0.25))) * f58)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f21) * VARYING0.w) + ((mix(f71, textureLod(PrefilteredEnvTexture, f69, f68).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f56.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f33)) * f75) * f55));
    vec4 f87 = vec4(0.0);
    f87.x = f86.x;
    vec4 f88 = f87;
    f88.y = f86.y;
