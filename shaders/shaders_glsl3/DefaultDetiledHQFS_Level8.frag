...
void main()
{
    ...
    vec4 f42 = f41;
    f42.z = f39.z;
    vec3 f43 = VARYING7.xyz - (CB0[16].xyz * VARYING3.w);
    float f44 = clamp(dot(step(CB0[24].xyz, abs(VARYING3.xyz - CB0[23].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f51 = f50.x;
    float f52 = f50.y;
    vec3 f53 = f43 - CB0[46].xyz;
    vec3 f54 = f43 - CB0[47].xyz;
    vec3 f55 = f43 - CB0[48].xyz;
    vec4 f56 = vec4(f43, 1.0) * mat4(CB8[((dot(f53, f53) < CB0[46].w) ? 0 : ((dot(f54, f54) < CB0[47].w) ? 1 : ((dot(f55, f55) < CB0[48].w) ? 2 : 3))) * 4 + 0], CB8[((dot(f53, f53) < CB0[46].w) ? 0 : ((dot(f54, f54) < CB0[47].w) ? 1 : ((dot(f55, f55) < CB0[48].w) ? 2 : 3))) * 4 + 1], CB8[((dot(f53, f53) < CB0[46].w) ? 0 : ((dot(f54, f54) < CB0[47].w) ? 1 : ((dot(f55, f55) < CB0[48].w) ? 2 : 3))) * 4 + 2], CB8[((dot(f53, f53) < CB0[46].w) ? 0 : ((dot(f54, f54) < CB0[47].w) ? 1 : ((dot(f55, f55) < CB0[48].w) ? 2 : 3))) * 4 + 3]);
    ...
    float f61 = exp(CB0[51].z * f60);
    float f62 = -exp((-CB0[51].w) * f60);
    vec2 f63 = (f59 * CB0[52].y) * vec2(f61, f62);
    vec2 f64 = f63 * f63;
    float f65 = f57.x;
    ...
    float f77 = clamp((f37 * CB0[14].w) * (((f37 * CB0[52].x) > 0.0) ? mix(min((f61 <= f65) ? 1.0 : clamp(((f66 / (f66 + (f67 * f67))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f62 <= f68) ? 1.0 : clamp(((f69 / (f69 + (f70 * f70))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f52, clamp((length(f43 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f52), 0.0, 1.0);
    float f78 = f72 * f72;
    ...
    float f106 = f100.y;
    float f107 = f100.z;
    vec3 f108 = (((((((f89 - (f84 * f88)) * CB0[15].xyz) * f77) + ((f49.xyz * (f49.w * 120.0)) * 1.0)) + ((f89 - (f97 * f88)) * (((((((CB0[40].xyz * f102) + (CB0[42].xyz * f103)) + (CB0[44].xyz * f104)) + (CB0[41].xyz * f105)) + (CB0[43].xyz * f106)) + (CB0[45].xyz * f107)) + (((((((CB0[34].xyz * f102) + (CB0[36].xyz * f103)) + (CB0[38].xyz * f104)) + (CB0[35].xyz * f105)) + (CB0[37].xyz * f106)) + (CB0[39].xyz * f107)) * f51)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f51))) * f42.xyz) + ((((f84 * (((f85 + (f85 * f85)) / (((f86 * f86) * ((f80 * 3.0) + 0.5)) * ((f79 * 0.75) + 0.25))) * f77)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) + ((mix(f93, textureLod(PrefilteredEnvTexture, f91, f90).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f73.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f51)) * f97) * f71));
    vec4 f109 = vec4(0.0);
    f109.x = f108.x;
    vec4 f110 = f109;
    f110.y = f108.y;
