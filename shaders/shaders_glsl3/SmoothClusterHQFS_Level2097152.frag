...
void main()
{
    ...
    float f29 = CB0[31].w * f14;
    vec3 f30 = reflect(-f27, f15);
    vec3 f31 = -CB0[16].xyz;
    float f32 = (dot(f15, f31) * CB0[14].w) * ((1.0 - ((step(f25.x, f26) * clamp(CB0[29].z + (CB0[29].w * abs(f26 - 0.5)), 0.0, 1.0)) * f25.y)) * f23.y);
    vec3 f33 = normalize(f27 - CB0[16].xyz);
    ...
    float f60 = f54.y;
    float f61 = f54.z;
    vec3 f62 = (((((((((vec3(1.0) - (f41 * f29)) * CB0[15].xyz) * f34) + (CB0[17].xyz * clamp(-f32, 0.0, 1.0))) + ((f22.xyz * (f22.w * 120.0)) * 1.0)) + ((vec3(1.0) - (f51 * f29)) * (((((((CB0[40].xyz * f56) + (CB0[42].xyz * f57)) + (CB0[44].xyz * f58)) + (CB0[41].xyz * f59)) + (CB0[43].xyz * f60)) + (CB0[45].xyz * f61)) + (((((((CB0[34].xyz * f56) + (CB0[36].xyz * f57)) + (CB0[38].xyz * f58)) + (CB0[35].xyz * f59)) + (CB0[37].xyz * f60)) + (CB0[39].xyz * f61)) * f24)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f24))) + vec3((f5.z * 2.0) * f14)) * (f13 * f13)) + ((((((f41 * (((f42 + (f42 * f42)) / (((f43 * f43) * ((f37 * 3.0) + 0.5)) * ((f36 * 0.75) + 0.25))) * f34)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f14) * VARYING0.w) + ((mix(f47, textureLod(PrefilteredEnvTexture, f45, f44).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f30.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f24)) * f51) * f29));
    vec4 f63 = vec4(0.0);
    f63.x = f62.x;
    vec4 f64 = f63;
    f64.y = f62.y;
