...
void main()
{
    ...
    float f48 = CB0[31].w * f12;
    vec3 f49 = reflect(-f44, f13);
    vec3 f50 = normalize(f44 - CB0[16].xyz);
    float f51 = clamp((f15 * CB0[14].w) * (((f15 * CB0[52].x) > 0.0) ? mix(min((f34 <= f38) ? 1.0 : clamp(((f39 / (f39 + (f40 * f40))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f35 <= f41) ? 1.0 : clamp(((f42 / (f42 + (f43 * f43))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f25, clamp((length(f16 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f25), 0.0, 1.0);
    float f52 = f45 * f45;
    ...
    float f80 = f74.y;
    float f81 = f74.z;
    vec3 f82 = ((((((((f63 - (f58 * f62)) * CB0[15].xyz) * f51) + ((f22.xyz * (f22.w * 120.0)) * 1.0)) + ((f63 - (f71 * f62)) * (((((((CB0[40].xyz * f76) + (CB0[42].xyz * f77)) + (CB0[44].xyz * f78)) + (CB0[41].xyz * f79)) + (CB0[43].xyz * f80)) + (CB0[45].xyz * f81)) + (((((((CB0[34].xyz * f76) + (CB0[36].xyz * f77)) + (CB0[38].xyz * f78)) + (CB0[35].xyz * f79)) + (CB0[37].xyz * f80)) + (CB0[39].xyz * f81)) * f24)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f24))) + vec3((f5.z * 2.0) * f12)) * f10) + ((((((f58 * (((f59 + (f59 * f59)) / (((f60 * f60) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25))) * f51)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f12) * VARYING0.w) + ((mix(f67, textureLod(PrefilteredEnvTexture, f65, f64).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f49.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f24)) * f71) * f48));
    vec4 f83 = vec4(0.0);
    f83.x = f82.x;
    vec4 f84 = f83;
    f84.y = f82.y;
