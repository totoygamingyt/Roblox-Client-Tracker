...
void main()
{
    ...
    float f38 = CB0[31].w * f0;
    float f39 = max(9.9999997473787516355514526367188e-05, dot(f21, f36));
    vec3 f40 = reflect(-f36, f21);
    float f41 = f37 * 5.0;
    vec3 f42 = vec4(f40, f41).xyz;
    vec3 f43 = textureLod(PrefilteredEnvTexture, f42, f41).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f40.y * 1.58823525905609130859375, 0.0, 1.0)));
    ...
    {
        f45 = mix(f44, textureLod(PrefilteredEnvBlendTargetTexture, f42, f41).xyz, vec3(CB0[32].w));
    }
    vec4 f46 = texture(PrecomputedBRDFTexture, vec2(f37, f39));
    float f47 = CB0[14].w * CB0[14].w;
    vec3 f48 = -CB0[16].xyz;
    float f49 = (dot(f21, f48) * CB0[14].w) * ((1.0 - ((step(f32.x, f33) * clamp(CB0[29].z + (CB0[29].w * abs(f33 - 0.5)), 0.0, 1.0)) * f32.y)) * f29.y);
    vec3 f50 = normalize(f36 - CB0[16].xyz);
    ...
    float f75 = f68.z;
    float f76 = 1.0 - f39;
    vec4 f77 = mix(vec4(mix((((((((vec3(1.0) - (f58 * f38)) * CB0[15].xyz) * f51) + (CB0[17].xyz * clamp(-f49, 0.0, 1.0))) + (f30 * f17)) + (((vec3(1.0) - (f65 * f38)) * (((((((CB0[40].xyz * f70) + (CB0[42].xyz * f71)) + (CB0[44].xyz * f72)) + (CB0[41].xyz * f73)) + (CB0[43].xyz * f74)) + (CB0[45].xyz * f75)) + (((((((CB0[34].xyz * f70) + (CB0[36].xyz * f71)) + (CB0[38].xyz * f72)) + (CB0[35].xyz * f73)) + (CB0[37].xyz * f74)) + (CB0[39].xyz * f75)) * f31))) * f17)) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f31))) * (f34 * f17), f62, vec3(VARYING7.w)) * f17, f17), vec4(f62, 1.0), vec4(((f76 * f76) * 0.800000011920928955078125) * clamp(f16, 0.0, 1.0))) + vec4((((f58 * (((f59 + (f59 * f59)) / (((f60 * f60) * ((f54 * 3.0) + 0.5)) * ((f53 * 0.75) + 0.25))) * f51)) * CB0[15].xyz) * f47) + (((mix(f45, f43, f61) * f65) * f38) * (f47 * f17)), 0.0);
    float f78 = clamp(exp2((CB0[18].z * f35) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
    vec3 f79 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[18].y, f78) * 5.0).xyz;
