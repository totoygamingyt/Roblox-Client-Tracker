...
void main()
{
    ...
    vec3 f13 = VARYING5.xyz - (CB0[16].xyz * 0.001000000047497451305389404296875);
    ...
    float f28 = CB0[31].w * f11;
    vec3 f29 = reflect(-f24, f12);
    vec3 f30 = -CB0[16].xyz;
    float f31 = (dot(f12, f30) * CB0[14].w) * ((1.0 - ((step(f22.x, f23) * clamp(CB0[29].z + (CB0[29].w * abs(f23 - 0.5)), 0.0, 1.0)) * f22.y)) * f20.y);
    vec3 f32 = normalize(f24 - CB0[16].xyz);
    ...
    float f62 = f56.y;
    float f63 = f56.z;
    vec3 f64 = (((((((((f45 - (f40 * f44)) * CB0[15].xyz) * f33) + (CB0[17].xyz * (f43 * clamp(-f31, 0.0, 1.0)))) + ((f19.xyz * (f19.w * 120.0)) * 1.0)) + ((f45 - (f53 * f44)) * (((((((CB0[40].xyz * f58) + (CB0[42].xyz * f59)) + (CB0[44].xyz * f60)) + (CB0[41].xyz * f61)) + (CB0[43].xyz * f62)) + (CB0[45].xyz * f63)) + (((((((CB0[34].xyz * f58) + (CB0[36].xyz * f59)) + (CB0[38].xyz * f60)) + (CB0[35].xyz * f61)) + (CB0[37].xyz * f62)) + (CB0[39].xyz * f63)) * f21)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f21))) + vec3((f5.z * 2.0) * f11)) * f10) + ((((((f40 * (((f41 + (f41 * f41)) / (((f42 * f42) * ((f36 * 3.0) + 0.5)) * ((f35 * 0.75) + 0.25))) * f33)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f11) * VARYING0.w) + ((mix(f49, textureLod(PrefilteredEnvTexture, f47, f46).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f29.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f21)) * f53) * f28));
    vec4 f65 = vec4(0.0);
    f65.x = f64.x;
    vec4 f66 = f65;
    f66.y = f64.y;
