...
void main()
{
    ...
    float f79 = CB0[31].w * f38;
    vec3 f80 = reflect(-f77, f46);
    vec3 f81 = normalize(f77 - CB0[16].xyz);
    float f82 = clamp((f48 * CB0[14].w) * (((f48 * CB0[52].x) > 0.0) ? mix(min((f67 <= f71) ? 1.0 : clamp(((f72 / (f72 + (f73 * f73))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f68 <= f74) ? 1.0 : clamp(((f75 / (f75 + (f76 * f76))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f58, clamp((length(f49 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f58), 0.0, 1.0);
    float f83 = f78 * f78;
    ...
    float f108 = f102.y;
    float f109 = f102.z;
    vec3 f110 = ((((((((vec3(1.0) - (f89 * f79)) * CB0[15].xyz) * f82) + ((f55.xyz * (f55.w * 120.0)) * 1.0)) + ((vec3(1.0) - (f99 * f79)) * (((((((CB0[40].xyz * f104) + (CB0[42].xyz * f105)) + (CB0[44].xyz * f106)) + (CB0[41].xyz * f107)) + (CB0[43].xyz * f108)) + (CB0[45].xyz * f109)) + (((((((CB0[34].xyz * f104) + (CB0[36].xyz * f105)) + (CB0[38].xyz * f106)) + (CB0[35].xyz * f107)) + (CB0[37].xyz * f108)) + (CB0[39].xyz * f109)) * f57)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f57))) + vec3((f28.z * 2.0) * f38)) * (f36 * f36)) + ((((((f89 * (((f90 + (f90 * f90)) / (((f91 * f91) * ((f85 * 3.0) + 0.5)) * ((f84 * 0.75) + 0.25))) * f82)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f38) * VARYING0.w) + ((mix(f95, textureLod(PrefilteredEnvTexture, f93, f92).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f80.y * 1.58823525905609130859375, 0.0, 1.0))), vec3(f57)) * f99) * f79));
    vec4 f111 = vec4(0.0);
    f111.x = f110.x;
    vec4 f112 = f111;
    f112.y = f110.y;
