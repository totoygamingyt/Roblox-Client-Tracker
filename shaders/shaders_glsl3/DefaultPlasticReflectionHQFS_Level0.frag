...
void main()
{
    ...
    vec3 f39 = textureLod(PrefilteredEnvTexture, f38, f37).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f36.y * 1.58823525905609130859375, 0.0, 1.0)));
    ...
    {
        f41 = mix(f40, textureLod(PrefilteredEnvBlendTargetTexture, f38, f37).xyz, vec3(CB0[32].w));
    }
    vec4 f42 = texture(PrecomputedBRDFTexture, vec2(f35, max(9.9999997473787516355514526367188e-05, dot(f17, f1))));
    vec3 f43 = -CB0[16].xyz;
    float f44 = (dot(f17, f43) * CB0[14].w) * ((1.0 - ((step(f32.x, f33) * clamp(CB0[29].z + (CB0[29].w * abs(f33 - 0.5)), 0.0, 1.0)) * f32.y)) * f30.y);
    ...
    float f67 = f61.y;
    float f68 = f61.z;
    vec3 f69 = ((((((((vec3(1.0) - (f53 * f34)) * CB0[15].xyz) * f46) + (CB0[17].xyz * clamp(-f44, 0.0, 1.0))) + ((f29.xyz * (f29.w * 120.0)) * 1.0)) + ((vec3(1.0) - (f58 * f34)) * (((((((CB0[40].xyz * f63) + (CB0[42].xyz * f64)) + (CB0[44].xyz * f65)) + (CB0[41].xyz * f66)) + (CB0[43].xyz * f67)) + (CB0[45].xyz * f68)) + (((((((CB0[34].xyz * f63) + (CB0[36].xyz * f64)) + (CB0[38].xyz * f65)) + (CB0[35].xyz * f66)) + (CB0[37].xyz * f67)) + (CB0[39].xyz * f68)) * f31)))) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f31))) * mix(f22.xyz, f39, vec3(VARYING7.w))) + ((((f53 * (((f54 + (f54 * f54)) / (((f55 * f55) * ((f49 * 3.0) + 0.5)) * ((f48 * 0.75) + 0.25))) * f46)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) + ((mix(f41, f39, vec3(f31)) * f58) * f34));
    vec4 f70 = vec4(0.0);
    f70.x = f69.x;
    vec4 f71 = f70;
    f71.y = f69.y;
    ...
    vec3 f75 = textureLod(PrefilteredEnvTexture, vec4(-VARYING4.xyz, 0.0).xyz, max(CB0[18].y, f74) * 5.0).xyz;
