...
void main()
{
    ...
    float f57 = CB0[31].w * f0;
    float f58 = max(9.9999997473787516355514526367188e-05, dot(f21, f55));
    vec3 f59 = reflect(-f55, f21);
    float f60 = f56 * 5.0;
    vec3 f61 = vec4(f59, f60).xyz;
    vec3 f62 = textureLod(PrefilteredEnvTexture, f61, f60).xyz * mix(CB0[31].xyz, CB0[30].xyz, vec3(clamp(f59.y * 1.58823525905609130859375, 0.0, 1.0)));
    ...
    {
        f64 = mix(f63, textureLod(PrefilteredEnvBlendTargetTexture, f61, f60).xyz, vec3(CB0[32].w));
    }
    vec4 f65 = texture(PrecomputedBRDFTexture, vec2(f56, f58));
    float f66 = CB0[14].w * CB0[14].w;
    vec3 f67 = normalize(f55 - CB0[16].xyz);
    float f68 = clamp((f23 * CB0[14].w) * (((f23 * CB0[52].x) > 0.0) ? mix(min((f43 <= f47) ? 1.0 : clamp(((f48 / (f48 + (f49 * f49))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0), (f44 <= f50) ? 1.0 : clamp(((f51 / (f51 + (f52 * f52))) - 0.20000000298023223876953125) * 1.25, 0.0, 1.0)), f34, clamp((length(f24 - CB0[11].xyz) * CB0[51].y) - (CB0[51].x * CB0[51].y), 0.0, 1.0)) : f34), 0.0, 1.0);
    float f69 = f56 * f56;
    ...
    float f92 = f85.z;
    float f93 = 1.0 - f58;
    vec4 f94 = mix(vec4(mix(((((((vec3(1.0) - (f75 * f57)) * CB0[15].xyz) * f68) + (f32 * f17)) + (((vec3(1.0) - (f82 * f57)) * (((((((CB0[40].xyz * f87) + (CB0[42].xyz * f88)) + (CB0[44].xyz * f89)) + (CB0[41].xyz * f90)) + (CB0[43].xyz * f91)) + (CB0[45].xyz * f92)) + (((((((CB0[34].xyz * f87) + (CB0[36].xyz * f88)) + (CB0[38].xyz * f89)) + (CB0[35].xyz * f90)) + (CB0[37].xyz * f91)) + (CB0[39].xyz * f92)) * f33))) * f17)) + (CB0[32].xyz + ((CB0[33].xyz * (2.0 - CB0[14].w)) * f33))) * (f53 * f17), f79, vec3(VARYING7.w)) * f17, f17), vec4(f79, 1.0), vec4(((f93 * f93) * 0.800000011920928955078125) * clamp(f16, 0.0, 1.0))) + vec4((((f75 * (((f76 + (f76 * f76)) / (((f77 * f77) * ((f71 * 3.0) + 0.5)) * ((f70 * 0.75) + 0.25))) * f68)) * CB0[15].xyz) * f66) + (((mix(f64, f62, f78) * f82) * f57) * (f66 * f17)), 0.0);
    float f95 = clamp(exp2((CB0[18].z * f54) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
