...
void main()
{
    ...
    vec3 f19 = -CB0[16].xyz;
    float f20 = (dot(f2, f19) * CB0[14].w) * ((1.0 - ((step(f14.x, f15) * clamp(CB0[29].z + (CB0[29].w * abs(f15 - 0.5)), 0.0, 1.0)) * f14.y)) * f13.y);
    ...
    float f32 = 1.0 - f18;
    vec3 f33 = ((((((vec3(f32) - (f29 * (f16 * f32))) * CB0[15].xyz) * f22) + (CB0[17].xyz * (f32 * clamp(-f20, 0.0, 1.0)))) + min((f12.xyz * (f12.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f13.x)), vec3(CB0[21].w))) * f7.xyz) + (((f29 * (((f30 + (f30 * f30)) / (((f31 * f31) * ((f25 * 3.0) + 0.5)) * ((f24 * 0.75) + 0.25))) * f22)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f34 = vec4(0.0);
    f34.x = f33.x;
    vec4 f35 = f34;
    f35.y = f33.y;
