...
void main()
{
    ...
    float f25 = f8.x;
    vec3 f26 = -CB0[16].xyz;
    float f27 = (dot(f15, f26) * CB0[14].w) * ((1.0 - ((step(f22.x, f23) * clamp(CB0[29].z + (CB0[29].w * abs(f23 - 0.5)), 0.0, 1.0)) * f22.y)) * f21.y);
    ...
    float f39 = 1.0 - f25;
    vec3 f40 = (((((((vec3(f39) - (f36 * ((CB0[31].w * f14) * f39))) * CB0[15].xyz) * f29) + (CB0[17].xyz * (f39 * clamp(-f27, 0.0, 1.0)))) + (min((f20.xyz * (f20.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f21.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * f13) + (((((f36 * (((f37 + (f37 * f37)) / (((f38 * f38) * ((f32 * 3.0) + 0.5)) * ((f31 * 0.75) + 0.25))) * f29)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f14) * VARYING0.w);
    vec4 f41 = vec4(0.0);
    f41.x = f40.x;
    vec4 f42 = f41;
    f42.y = f40.y;
