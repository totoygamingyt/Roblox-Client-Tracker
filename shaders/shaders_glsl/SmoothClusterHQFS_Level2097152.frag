...
void main()
{
    ...
    vec3 f25 = -CB0[16].xyz;
    float f26 = (dot(f15, f25) * CB0[14].w) * ((1.0 - ((step(f22.x, f23) * clamp(CB0[29].z + (CB0[29].w * abs(f23 - 0.5)), 0.0, 1.0)) * f22.y)) * f21.y);
    ...
    vec3 f38 = (((((((vec3(1.0) - (f35 * (CB0[31].w * f14))) * CB0[15].xyz) * f28) + (CB0[17].xyz * clamp(-f26, 0.0, 1.0))) + (min((f20.xyz * (f20.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f21.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * (f13 * f13)) + (((((f35 * (((f36 + (f36 * f36)) / (((f37 * f37) * ((f31 * 3.0) + 0.5)) * ((f30 * 0.75) + 0.25))) * f28)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f14) * VARYING0.w);
    vec4 f39 = vec4(0.0);
    f39.x = f38.x;
    vec4 f40 = f39;
    f40.y = f38.y;
