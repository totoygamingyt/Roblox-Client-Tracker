...
void main()
{
    ...
    vec3 f34 = -CB0[16].xyz;
    float f35 = (dot(f24, f34) * CB0[14].w) * ((1.0 - ((step(f31.x, f32) * clamp(CB0[29].z + (CB0[29].w * abs(f32 - 0.5)), 0.0, 1.0)) * f31.y)) * f30.y);
    ...
    vec3 f47 = (((((((vec3(1.0) - (f44 * (CB0[31].w * f23))) * CB0[15].xyz) * f37) + (CB0[17].xyz * clamp(-f35, 0.0, 1.0))) + (min((f29.xyz * (f29.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f30.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f23)) * (f22.xyz * f22.xyz)) + (((((f44 * (((f45 + (f45 * f45)) / (((f46 * f46) * ((f40 * 3.0) + 0.5)) * ((f39 * 0.75) + 0.25))) * f37)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f23) * VARYING0.w);
    vec4 f48 = vec4(0.0);
    f48.x = f47.x;
    vec4 f49 = f48;
    f49.y = f47.y;
