...
void main()
{
    ...
    float f63 = f14.x;
    vec3 f64 = -CB0[16].xyz;
    float f65 = (dot(f53, f64) * CB0[14].w) * ((1.0 - ((step(f60.x, f61) * clamp(CB0[29].z + (CB0[29].w * abs(f61 - 0.5)), 0.0, 1.0)) * f60.y)) * f59.y);
    ...
    float f77 = 1.0 - f63;
    vec3 f78 = (((((((vec3(f77) - (f74 * ((CB0[31].w * f52) * f77))) * CB0[15].xyz) * f67) + (CB0[17].xyz * (f77 * clamp(-f65, 0.0, 1.0)))) + (min((f58.xyz * (f58.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f59.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f14.z * 2.0) * f52)) * f51) + (((((f74 * (((f75 + (f75 * f75)) / (((f76 * f76) * ((f70 * 3.0) + 0.5)) * ((f69 * 0.75) + 0.25))) * f67)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f52) * VARYING0.w);
    vec4 f79 = vec4(0.0);
    f79.x = f78.x;
    vec4 f80 = f79;
    f80.y = f78.y;
