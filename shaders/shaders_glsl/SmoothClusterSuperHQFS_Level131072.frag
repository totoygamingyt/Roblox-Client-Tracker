...
void main()
{
    ...
    vec3 f57 = VARYING5.xyz - (CB0[16].xyz * 0.001000000047497451305389404296875);
    float f58 = clamp(dot(step(CB0[24].xyz, abs(VARYING4 - CB0[23].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    float f66 = f14.x;
    vec3 f67 = -CB0[16].xyz;
    float f68 = (dot(f56, f67) * CB0[14].w) * ((1.0 - ((step(f63.x, f64) * clamp(CB0[29].z + (CB0[29].w * abs(f64 - 0.5)), 0.0, 1.0)) * f63.y)) * f62.y);
    ...
    float f80 = 1.0 - f66;
    vec3 f81 = (((((((vec3(f80) - (f77 * ((CB0[31].w * f52) * f80))) * CB0[15].xyz) * f70) + (CB0[17].xyz * (f80 * clamp(-f68, 0.0, 1.0)))) + (min((f61.xyz * (f61.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f62.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f14.z * 2.0) * f52)) * f51) + (((((f77 * (((f78 + (f78 * f78)) / (((f79 * f79) * ((f73 * 3.0) + 0.5)) * ((f72 * 0.75) + 0.25))) * f70)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f52) * VARYING0.w);
    vec4 f82 = vec4(0.0);
    f82.x = f81.x;
    vec4 f83 = f82;
    f83.y = f81.y;
