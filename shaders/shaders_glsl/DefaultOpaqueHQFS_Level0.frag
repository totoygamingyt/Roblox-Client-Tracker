...
void main()
{
    ...
    vec3 f18 = -CB0[16].xyz;
    float f19 = (dot(f1, f18) * CB0[14].w) * ((1.0 - ((step(f13.x, f14) * clamp(CB0[29].z + (CB0[29].w * abs(f14 - 0.5)), 0.0, 1.0)) * f13.y)) * f12.y);
    ...
    float f31 = 1.0 - f17;
    vec3 f32 = ((((((vec3(f31) - (f28 * (f15 * f31))) * CB0[15].xyz) * f21) + (CB0[17].xyz * (f31 * clamp(-f19, 0.0, 1.0)))) + min((f11.xyz * (f11.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f12.x)), vec3(CB0[21].w))) * f6.xyz) + (((f28 * (((f29 + (f29 * f29)) / (((f30 * f30) * ((f24 * 3.0) + 0.5)) * ((f23 * 0.75) + 0.25))) * f21)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f33 = vec4(0.0);
    f33.x = f32.x;
    vec4 f34 = f33;
    f34.y = f32.y;
