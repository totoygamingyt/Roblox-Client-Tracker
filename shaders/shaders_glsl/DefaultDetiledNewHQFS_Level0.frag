...
void main()
{
    ...
    float f45 = f38.z;
    float f46 = CB0[31].w * f1;
    float f47 = 0.08900000154972076416015625 + (f27.y * 0.9110000133514404296875);
    float f48 = f27.x * f46;
    vec3 f49 = -CB0[16].xyz;
    float f50 = (dot(f32, f49) * CB0[14].w) * ((1.0 - ((step(f44.x, f45) * clamp(CB0[29].z + (CB0[29].w * abs(f45 - 0.5)), 0.0, 1.0)) * f44.y)) * f43.y);
    ...
    float f62 = 1.0 - f48;
    vec3 f63 = ((((((vec3(f62) - (f59 * (f46 * f62))) * CB0[15].xyz) * f52) + (CB0[17].xyz * (f62 * clamp(-f50, 0.0, 1.0)))) + min((f42.xyz * (f42.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f43.x)), vec3(CB0[21].w))) * f37.xyz) + (((f59 * (((f60 + (f60 * f60)) / (((f61 * f61) * ((f55 * 3.0) + 0.5)) * ((f54 * 0.75) + 0.25))) * f52)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f64 = vec4(0.0);
    f64.x = f63.x;
    vec4 f65 = f64;
    f65.y = f63.y;
