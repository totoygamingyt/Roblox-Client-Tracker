...
void main()
{
    ...
    vec3 f28 = -CB0[16].xyz;
    float f29 = (dot(f18, f28) * CB0[14].w) * ((1.0 - ((step(f25.x, f26) * clamp(CB0[29].z + (CB0[29].w * abs(f26 - 0.5)), 0.0, 1.0)) * f25.y)) * f24.y);
    ...
    vec3 f41 = (((((((vec3(1.0) - (f38 * (CB0[31].w * f14))) * CB0[15].xyz) * f31) + (CB0[17].xyz * clamp(-f29, 0.0, 1.0))) + (min((f23.xyz * (f23.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f24.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * (f13 * f13)) + (((((f38 * (((f39 + (f39 * f39)) / (((f40 * f40) * ((f34 * 3.0) + 0.5)) * ((f33 * 0.75) + 0.25))) * f31)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f14) * VARYING0.w);
    vec4 f42 = vec4(0.0);
    f42.x = f41.x;
    vec4 f43 = f42;
    f43.y = f41.y;
