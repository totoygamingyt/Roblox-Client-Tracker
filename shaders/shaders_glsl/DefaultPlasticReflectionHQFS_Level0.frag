...
void main()
{
    ...
    vec3 f32 = -CB0[16].xyz;
    float f33 = (dot(f16, f32) * CB0[14].w) * ((1.0 - ((step(f28.x, f29) * clamp(CB0[29].z + (CB0[29].w * abs(f29 - 0.5)), 0.0, 1.0)) * f28.y)) * f27.y);
    ...
    vec3 f45 = ((((((vec3(1.0) - (f42 * (CB0[31].w * f3))) * CB0[15].xyz) * f35) + (CB0[17].xyz * clamp(-f33, 0.0, 1.0))) + min((f26.xyz * (f26.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f27.x)), vec3(CB0[21].w))) * mix(f21.xyz, (f31 * f31) * CB0[20].x, vec3(VARYING7.w))) + (((f42 * (((f43 + (f43 * f43)) / (((f44 * f44) * ((f38 * 3.0) + 0.5)) * ((f37 * 0.75) + 0.25))) * f35)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f46 = vec4(0.0);
    f46.x = f45.x;
    vec4 f47 = f46;
    f47.y = f45.y;
