...
void main()
{
    ...
    float f46 = f39.z;
    float f47 = CB0[31].w * f1;
    float f48 = 0.08900000154972076416015625 + (f28.y * 0.9110000133514404296875);
    float f49 = f28.x * f47;
    vec3 f50 = -CB0[16].xyz;
    float f51 = (dot(f33, f50) * CB0[14].w) * ((1.0 - ((step(f45.x, f46) * clamp(CB0[29].z + (CB0[29].w * abs(f46 - 0.5)), 0.0, 1.0)) * f45.y)) * f44.y);
    ...
    float f63 = 1.0 - f49;
    vec3 f64 = ((((((vec3(f63) - (f60 * (f47 * f63))) * CB0[15].xyz) * f53) + (CB0[17].xyz * (f63 * clamp(-f51, 0.0, 1.0)))) + min((f43.xyz * (f43.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f44.x)), vec3(CB0[21].w))) * f38.xyz) + (((f60 * (((f61 + (f61 * f61)) / (((f62 * f62) * ((f56 * 3.0) + 0.5)) * ((f55 * 0.75) + 0.25))) * f53)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f65 = vec4(0.0);
    f65.x = f64.x;
    vec4 f66 = f65;
    f66.y = f64.y;
