...
void main()
{
    ...
    vec3 f30 = -CB0[16].xyz;
    float f31 = (dot(f15, f30) * CB0[14].w) * ((1.0 - ((step(f27.x, f28) * clamp(CB0[29].z + (CB0[29].w * abs(f28 - 0.5)), 0.0, 1.0)) * f27.y)) * f26.y);
    ...
    vec3 f43 = ((((((vec3(1.0) - (f40 * (CB0[31].w * f2))) * CB0[15].xyz) * f33) + (CB0[17].xyz * clamp(-f31, 0.0, 1.0))) + min((f25.xyz * (f25.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f26.x)), vec3(CB0[21].w))) * f20.xyz) + (((f40 * (((f41 + (f41 * f41)) / (((f42 * f42) * ((f36 * 3.0) + 0.5)) * ((f35 * 0.75) + 0.25))) * f33)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f44 = vec4(0.0);
    f44.x = f43.x;
    vec4 f45 = f44;
    f45.y = f43.y;
