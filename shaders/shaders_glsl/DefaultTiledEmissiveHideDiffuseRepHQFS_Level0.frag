...
void main()
{
    ...
    float f31 = f24.z;
    float f32 = CB0[31].w * f1;
    float f33 = 0.08900000154972076416015625 + (f13.y * 0.9110000133514404296875);
    float f34 = f13.x * f32;
    vec3 f35 = -CB0[16].xyz;
    float f36 = (dot(f18, f35) * CB0[14].w) * ((1.0 - ((step(f30.x, f31) * clamp(CB0[29].z + (CB0[29].w * abs(f31 - 0.5)), 0.0, 1.0)) * f30.y)) * f29.y);
    ...
    float f48 = 1.0 - f34;
    vec3 f49 = (((((((vec3(f48) - (f45 * (f32 * f48))) * CB0[15].xyz) * f38) + (CB0[17].xyz * (f48 * clamp(-f36, 0.0, 1.0)))) + min((f28.xyz * (f28.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f29.x)), vec3(CB0[21].w))) + vec3(((1.0 - f13.w) * 2.0) * f1)) * f23.xyz) + (((f45 * (((f46 + (f46 * f46)) / (((f47 * f47) * ((f41 * 3.0) + 0.5)) * ((f40 * 0.75) + 0.25))) * f38)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f50 = vec4(0.0);
    f50.x = f49.x;
    vec4 f51 = f50;
    f51.y = f49.y;
