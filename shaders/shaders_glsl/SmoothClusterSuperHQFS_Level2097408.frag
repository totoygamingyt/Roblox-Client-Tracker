...
void main()
{
    ...
    vec3 f57 = -CB0[16].xyz;
    float f58 = (dot(f47, f57) * CB0[14].w) * ((1.0 - ((step(f54.x, f55) * clamp(CB0[29].z + (CB0[29].w * abs(f55 - 0.5)), 0.0, 1.0)) * f54.y)) * f53.y);
    ...
    vec3 f70 = (((((((vec3(1.0) - (f67 * (CB0[31].w * f43))) * CB0[15].xyz) * f60) + (CB0[17].xyz * clamp(-f58, 0.0, 1.0))) + (min((f52.xyz * (f52.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f53.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f43)) * (f42 * f42)) + (((((f67 * (((f68 + (f68 * f68)) / (((f69 * f69) * ((f63 * 3.0) + 0.5)) * ((f62 * 0.75) + 0.25))) * f60)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f43) * VARYING0.w);
    vec4 f71 = vec4(0.0);
    f71.x = f70.x;
    vec4 f72 = f71;
    f72.y = f70.y;
