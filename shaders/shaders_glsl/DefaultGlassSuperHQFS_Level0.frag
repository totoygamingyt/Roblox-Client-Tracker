...
void main()
{
    ...
    vec3 f35 = -CB0[16].xyz;
    float f36 = (dot(f19, f35) * CB0[14].w) * ((1.0 - ((step(f28.x, f29) * clamp(CB0[29].z + (CB0[29].w * abs(f29 - 0.5)), 0.0, 1.0)) * f28.y)) * f25.y);
    vec3 f37 = normalize(f32 - CB0[16].xyz);
    ...
    vec3 f48 = mix(f26, (f34 * f34) * CB0[20].x, vec3(f27)) * mix(vec3(1.0), f30, vec3(0.5));
    ...
    vec4 f79 = mix(mix(f78, vec4(mix((((((vec3(1.0) - (f45 * (CB0[31].w * f0))) * CB0[15].xyz) * f38) + (CB0[17].xyz * clamp(-f36, 0.0, 1.0))) + min(f26 + (CB0[13].xyz + (CB0[14].xyz * f27)), vec3(CB0[21].w))) * (f30 * f14), f48, vec3(VARYING7.w)), 1.0), vec4(f14)), vec4(f48, 1.0), vec4(((f49 * f49) * 0.800000011920928955078125) * f15)) + vec4(((f45 * (((f46 + (f46 * f46)) / (((f47 * f47) * ((f41 * 3.0) + 0.5)) * ((f40 * 0.75) + 0.25))) * f38)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w), 0.0);
    float f80 = clamp(exp2((CB0[18].z * f31) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
