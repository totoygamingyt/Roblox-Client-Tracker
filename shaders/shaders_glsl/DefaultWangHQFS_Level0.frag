...
void main()
{
    ...
    float f40 = f19.x * f38;
    vec3 f41 = -CB0[16].xyz;
    float f42 = (dot(f24, f41) * CB0[14].w) * ((1.0 - ((step(f36.x, f37) * clamp(CB0[29].z + (CB0[29].w * abs(f37 - 0.5)), 0.0, 1.0)) * f36.y)) * f35.y);
    ...
    float f54 = 1.0 - f40;
    vec3 f55 = ((((((vec3(f54) - (f51 * (f38 * f54))) * CB0[15].xyz) * f44) + (CB0[17].xyz * (f54 * clamp(-f42, 0.0, 1.0)))) + min((f34.xyz * (f34.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f35.x)), vec3(CB0[21].w))) * f29.xyz) + (((f51 * (((f52 + (f52 * f52)) / (((f53 * f53) * ((f47 * 3.0) + 0.5)) * ((f46 * 0.75) + 0.25))) * f44)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f56 = vec4(0.0);
    f56.x = f55.x;
    vec4 f57 = f56;
    f57.y = f55.y;
