...
void main()
{
    ...
    float f28 = f8.x;
    vec3 f29 = -CB0[16].xyz;
    float f30 = (dot(f18, f29) * CB0[14].w) * ((1.0 - ((step(f25.x, f26) * clamp(CB0[29].z + (CB0[29].w * abs(f26 - 0.5)), 0.0, 1.0)) * f25.y)) * f24.y);
    ...
    float f42 = 1.0 - f28;
    vec3 f43 = (((((((vec3(f42) - (f39 * ((CB0[31].w * f14) * f42))) * CB0[15].xyz) * f32) + (CB0[17].xyz * (f42 * clamp(-f30, 0.0, 1.0)))) + (min((f23.xyz * (f23.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f24.x)), vec3(CB0[21].w)) * 1.0)) + vec3((f8.z * 2.0) * f14)) * f13) + (((((f39 * (((f40 + (f40 * f40)) / (((f41 * f41) * ((f35 * 3.0) + 0.5)) * ((f34 * 0.75) + 0.25))) * f32)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w)) * f14) * VARYING0.w);
    vec4 f44 = vec4(0.0);
    f44.x = f43.x;
    vec4 f45 = f44;
    f45.y = f43.y;
