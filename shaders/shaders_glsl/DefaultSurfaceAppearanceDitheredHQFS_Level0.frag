...
void main()
{
    ...
    float f32 = f12.x * f30;
    vec3 f33 = -CB0[16].xyz;
    float f34 = (dot(f16, f33) * CB0[14].w) * ((1.0 - ((step(f28.x, f29) * clamp(CB0[29].z + (CB0[29].w * abs(f29 - 0.5)), 0.0, 1.0)) * f28.y)) * f27.y);
    ...
    float f46 = 1.0 - f32;
    vec3 f47 = ((((((vec3(f46) - (f43 * (f30 * f46))) * CB0[15].xyz) * f36) + (CB0[17].xyz * (f46 * clamp(-f34, 0.0, 1.0)))) + min((f26.xyz * (f26.w * 120.0)) + (CB0[13].xyz + (CB0[14].xyz * f27.x)), vec3(CB0[21].w))) * f21.xyz) + (((f43 * (((f44 + (f44 * f44)) / (((f45 * f45) * ((f39 * 3.0) + 0.5)) * ((f38 * 0.75) + 0.25))) * f36)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w));
    vec4 f48 = vec4(0.0);
    f48.x = f47.x;
    vec4 f49 = f48;
    f49.y = f47.y;
