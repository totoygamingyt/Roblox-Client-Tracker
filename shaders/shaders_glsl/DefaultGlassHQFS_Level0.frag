...
void main()
{
    ...
    vec3 f19 = VARYING7.xyz - (CB0[16].xyz * VARYING3.w);
    float f20 = clamp(dot(step(CB0[24].xyz, abs(VARYING3.xyz - CB0[23].xyz)), vec3(1.0)), 0.0, 1.0);
    ...
    vec3 f34 = -CB0[16].xyz;
    float f35 = (dot(f18, f34) * CB0[14].w) * ((1.0 - ((step(f27.x, f28) * clamp(CB0[29].z + (CB0[29].w * abs(f28 - 0.5)), 0.0, 1.0)) * f27.y)) * f24.y);
    vec3 f36 = normalize(f31 - CB0[16].xyz);
    ...
    vec3 f47 = mix(f25, (f33 * f33) * CB0[20].x, vec3(f26)) * mix(vec3(1.0), f29, vec3(0.5));
    float f48 = 1.0 - max(9.9999997473787516355514526367188e-05, dot(f18, f31));
    vec4 f49 = mix(vec4(mix((((((vec3(1.0) - (f44 * (CB0[31].w * f0))) * CB0[15].xyz) * f37) + (CB0[17].xyz * clamp(-f35, 0.0, 1.0))) + min(f25 + (CB0[13].xyz + (CB0[14].xyz * f26)), vec3(CB0[21].w))) * (f29 * f14), f47, vec3(VARYING7.w)) * f14, f14), vec4(f47, 1.0), vec4(((f48 * f48) * 0.800000011920928955078125) * clamp(f13, 0.0, 1.0))) + vec4(((f44 * (((f45 + (f45 * f45)) / (((f46 * f46) * ((f40 * 3.0) + 0.5)) * ((f39 * 0.75) + 0.25))) * f37)) * CB0[15].xyz) * (CB0[14].w * CB0[14].w), 0.0);
    float f50 = clamp(exp2((CB0[18].z * f30) + CB0[18].x) - CB0[18].w, 0.0, 1.0);
